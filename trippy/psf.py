...
class modelPSF:
    ...
    def __init__(self, x=-1, y=-1, alpha=-1, beta=-1, repFact=10, verbose=False, restore=False):
        ...
        if repFact not in [5, 10]:
            raise Warning(
                'This has only been robustly tested with repFact=5 or 10. I encourage you to stick with those.')
    ...
    def computeRoundAperCorrFromPSF(self, radii, useLookupTable=True, display=True, displayAperture=True):
        ...
        for iii in range(len(self.aperCorrRadii)):
            ...
            phot(B / 2., A / 2., radius=r * self.repFact, l=0., a=0., skyRadius=None, zpt=0.0, width=width,
    ...
    def roundAperCorr(self, r):
        ...
        if self.aperCorrFunc <> None:
            ...
            raise Exception(
                'Must first fun computeRoundAperCorrFromPSF before the aperture corrections can be evaluated here.')
    ...
    def computeLineAperCorrFromTSF(self, radii, l, a, display=True, displayAperture=True):
        ...
        for ii in range(len(self.lineAperCorrRadii)):
            ...
            phot(B / 2., A / 2., radius=r * self.repFact, l=l * self.repFact, a=a, skyRadius=None, zpt=0.0, width=width,
                 display=displayAperture)
            m = phot.magnitude
    ...
    def line(self, rate, angle, dt, pixScale=0.2, display=False, useLookupTable=True):
        ...
        if useLookupTable:
            ...
            self.longPSF = signal.convolve2d(self.moffProf + self.lookupTable * self.repFact * self.repFact,
                                             self.line2d, mode='same')
...
#                  fitXY=False, fitMaxRadius=-1.):
        ...
        if fitXY:
            ...
            for ii in range(len(deltaX)):
                for jj in range(len(deltaY)):
                    ...
                    lsqf = opti.leastsq(self._residFAB, (peakGuess), args=(self.alpha, self.beta, fitMaxRadius),
                                        maxfev=1000)
    ...
    def genLookupTable(self, imData, centXs, centYs, verbose=False, bpMask=None, threeSigCut=True):
        ...
        for ii in range(len(centXs)):
            ...
            cx, cy = adjCentXs[ii] - int(adjCentXs[ii]) + self.boxSize + 2, adjCentYs[ii] - int(
                adjCentYs[ii]) + self.boxSize + 2
            cx += 0.5
            cy += 0.5

            cut = imData[yint:yint + 2 * self.boxSize + 5, xint:xint + 2 * self.boxSize + 5]

            self.fitMoffat(cut, num.array([cx]), num.array([cy]), self.boxSize, verbose=verbose, fixAB=True,
                           fitXY=False, fitMaxRadius=3.)
            self.imData = num.copy(
                imData)  # this is necessary because the imdata gets set to the shifted image subsection
    ...
    def _resid(self, p, maxRad):
        ...
        self.alpha = alpha
        self.beta = beta
        err = (self.subSec - (self.bg + A * downSample2d(self.moffat(self.repRads), self.repFact))).reshape(
            self.subSec.size)
        # if maxRad>0:
