...
class starChooser:
    ...
    **To mark it bad, right click. Red marked sources are excluded from the fitting.**
    ...
    Further sources can be excluded by zooming the main panel. Only the blue sources within the zoom window are considered.

    It takes as input:
    -data: the image data loaded up through pyfits or astropy.io.fits
    -XWIN_IMAGE, YWIN_IMAGE, FLUX_AUTO, FLUXERR_AUTO are numpy arrays of the x,y coordinates, flux, and flux uncertainty
     of a user-provided list of candidate stars.
     I used the names of sextractor parameters because these can be easily extracted from sextractor output using those names.
    ...
    def __call__(self, moffatWidth, moffatSNR, initAlpha=5., initBeta=2., repFact=5, xWidth=51, yWidth=51,
        ...
        for j in range(len(self.XWIN_IMAGE)):
            if self.FLUX_AUTO[j] / self.FLUXERR_AUTO[j] > self.moffatSNR:
                if self.XWIN_IMAGE[j] - 1 < 0 or self.XWIN_IMAGE[j] - 1 >= self.data.shape[1] or self.YWIN_IMAGE[
                    j] - 1 < 0 or self.YWIN_IMAGE[j] - 1 >= self.data.shape[0]:
                    continue
                mpsf = psf.modelPSF(num.arange(xWidth), num.arange(yWidth), alpha=self.initAlpha, beta=self.initBeta,
                                    repFact=self.repFact)
                mpsf.fitMoffat(self.data, self.XWIN_IMAGE[j], self.YWIN_IMAGE[j], boxSize=self.moffatWidth,
                               verbose=verbose)
                ...
                if fwhm <> None and not (num.isnan(mpsf.beta) or num.isnan(mpsf.alpha)):
                    ...
                    print '{: 8.2f} {: 8.2f} {: 5.2f} {: 5.2f} {: 5.2f}'.format(self.XWIN_IMAGE[j], self.YWIN_IMAGE[j],
                                                                                mpsf.alpha, mpsf.beta, fwhm)
                    ...
                    self.points.append(
                        [fwhm, mpsf.chi, mpsf.alpha, mpsf.beta, self.XWIN_IMAGE[j], self.YWIN_IMAGE[j], mpsf.bg])
        ...
        w = num.where(
            (self.points[:, 0] > fwhm_lim[0]) & (self.points[:, 0] < fwhm_lim[1]) & (self.points[:, 1] > chi_lim[0]) & (
             self.points[:, 1] < chi_lim[1]) & (self.goodStars == True))
        pyl.close()
    ...
    def PSFrange(self, junkAx):
        ...
        w = num.where(
            (self.points[:, 0] >= self.psfPlotLimits[0][0]) & (self.points[:, 0] <= self.psfPlotLimits[0][1]) & (
            self.points[:, 1] >= self.psfPlotLimits[1][0]) & (self.points[:, 1] <= self.psfPlotLimits[1][1]))[0]
    ...
    def ScatterPSF(self, event):
        ...
        if me.button == 3:
            if self.goodStars[arg] == True:
                self.goodStars[arg] = False
            else:
                self.goodStars[arg] = True
